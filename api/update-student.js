    ...
    if (action === 'send-liability-invite') {
      ...
      for (const pid of pageIds) {
        try {
          ...
          const liabBtn = needLiab ? '<a href="' + link.replace(/=/g, '&#61;') + '" style="display:inline-block;background:#111114;color:white;padding:14px 32px;border-radius:14px;text-decoration:none;font-weight:700;font-size:15px;margin:0 4px 8px;">📝 안전 동의서 / Liability</a>' : '';
          const infoBtn = missing.length ? '<a href="' + infoLink.replace(/=/g, '&#61;') + '" style="display:inline-block;background:#2e9e5b;color:white;padding:14px 32px;border-radius:14px;text-decoration:none;font-weight:700;font-size:15px;margin:0 4px 8px;">📋 정보 입력 / Info Update</a>' : '';
